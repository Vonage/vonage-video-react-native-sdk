...
export default class OTSession extends Component {
  ...
  // Approach: each session-level `OT.onXxx` handler is a TurboModule
  // `EventEmitter` (see src/NativeOpentok.ts). Registering a listener returns
  // an EventSubscription exposing `.remove()`. We retain every subscription
  // here at registration time and call `.remove()` on each in
  // componentWillUnmount so handlers do not accumulate across mount cycles.
  _eventSubscriptions = [];
  ...
  async initSession(apiKey, sessionId, token) {
    ...
    this._eventSubscriptions.push(
      OT.onSessionDisconnected((event) => {
        ...
        dispatchEvent(sessionId, 'sessionDisconnected', event);
      })
    );
    OT.initSession(
      apiKey,
  ...
  componentWillUnmount() {
    // Tear down every retained native event subscription. Each entry is the
    // EventSubscription returned by an `OT.onXxx` registration and exposes
    // `.remove()`. Guarded so a malformed handle cannot break unmount.
    if (Array.isArray(this._eventSubscriptions)) {
